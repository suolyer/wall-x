...
from wall_x._vendor.x2robot_utils.geometry import (
    compute_delta_from_state_and_abs_rot,
)
...
class PreprocessedDataset(Dataset[T_co]):
    ...
    def _to_relative_action(self, action, agent_pos):
        ...
        Keys in ``dof_config`` ending with ``_relative`` (cartesian / rotation)
        are expressed relative to the proprioception slice at the same indices.
        Other keys are passed through unchanged. Virtual padding keys are omitted
        here and filled later by the collator.
        """
