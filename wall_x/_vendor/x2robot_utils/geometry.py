...
def compute_delta_from_state_and_abs_rot(
    rotations: np.ndarray, state: np.ndarray
) -> np.ndarray:
    """Absolute rotations in the action chunk → delta w.r.t. current agent_pos.

    Convention matches norm-stats and inference: R_rel = R_abs @ R_state^T.
    """
