...
class RobotRegistry:
    def __init__(self):
        ...
        self.logger.debug(f"Init robot registry with {len(self.registry)} robots")
...
class RobotCommunication:
    ...
    def connect(self):
        self.logger.info("Opening socket connections...")
        ...
        action_thread.join()
        self.logger.info("Socket connections ready")
    ...
    def recv_action_data(self):
        ...
        self.logger.debug(f"Action payload size: {data_size} bytes")
    ...
    def send_dict(self, dict_data):
        self.logger.debug(f"Sending dict with {len(dict_data)} fields")
        ...
        self.logger.debug(f"Send complete: {len(data_bytes)} bytes")

    def close(self):
        self.logger.info("Closing socket connections...")
...
class DummyRobotController:
    ...
    def recv_image(
        ...
        self.logger.debug(f"Load view data from pickle: {cam_names}")
        with open(self.views_path, "rb") as f:
            all_views = pickle.load(f)
        self.logger.debug(f"Views loaded: {len(all_views)}")
        return all_views
    ...
    def recv_action(self):
        ...
        self.logger.debug("Load action data from pickle")
