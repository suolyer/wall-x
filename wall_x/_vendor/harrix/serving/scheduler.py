...
class RequestScheduler:
    ...
    async def start(self):
        ...
        logger.info(f"RequestScheduler started: max_batch={self.max_batch_size}, max_wait={self.max_wait_time*1000}ms")
    ...
    async def stop(self):
        ...
        if remaining:
            logger.info(f"Graceful shutdown: processing {remaining} remaining request(s)")
        while True:
            async with self.queue_lock:
                if len(self.queue) == 0:
                    break
    ...
    async def add_request(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.queue_lock:
            if len(self.queue) >= self.max_queue_size:
                raise RuntimeError(f"Queue full: {len(self.queue)}/{self.max_queue_size}")
    ...
    async def _process_batch(self, batch: List[Request]):
        ...
        try:
            ...
            logger.info(f"Batch processed in {infer_time*1000:.1f}ms, throughput: {len(batch)/infer_time:.1f} req/s")
