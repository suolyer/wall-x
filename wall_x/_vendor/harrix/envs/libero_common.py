...
def encode_proprio(
    ...
    """
    agent_pos_config = _resolve_agent_pos_config(train_config)
...
def gripper_model_to_libero_osc(grip_2d: np.ndarray) -> float:
    ...
    cmd = float(np.clip(float(g[0]), -1.0, 1.0))
    if os.environ.get("WALLX_LIBERO_GRIPPER_BINARIZE", "1") == "1":
        if abs(cmd) < 1e-6:
            cmd = -1.0
        else:
...
def model_action_to_libero_env(action: np.ndarray) -> np.ndarray:
    """Convert model delta (pos3 + eulerΔ3 + gripper) to robosuite OSC_POSE 7D."""
