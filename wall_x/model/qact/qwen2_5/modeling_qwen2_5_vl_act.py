...
class Qwen2_5_VLMoEForAction(
        ...
        **kwargs,
        ...
        if labels is not None and self.training:
            ...
            if _lm_loss_mask.any():
                ...
                logits = self.lm_head(lm_hidden)  # [N_loss, V]
            else:
                logits = None
        else:
    ...
    def _execute_flow_action(self, ctx):
        ...
        Subclasses may override to inject an optimized backend.
        """
    ...
    def _generate_flow_action_vanilla(self, ctx):
        ...
        ctx["noisy_action"] = ctx["noisy_action"] + dt * v_0.reshape(
    ...
    def _finalize_flow_action_output(self, action_trajectory, ctx):
        ...
        predict_action = self.action_preprocessor.normalizer_action.unnormalize_data(
            predict_action, ctx["dataset_names"]
        )
        output = {"predict_action": predict_action}
        if ctx["action_chunk"] is not None:
            output["gt_action"] = (
                self.action_preprocessor.normalizer_action.unnormalize_data(
                    ctx["action_chunk"], ctx["dataset_names"]
                )
            )
    ...
    def convert_to_fsdp(
        ...
        *,
        ...
        use_dedicated_params: bool = False,
    ):
        ...
        When ``use_dedicated_params=True``, DMuon owns selected trainable
        matrix weights before the remaining parameters are wrapped by FSDP2.
        """
        from torch.distributed.fsdp import fully_shard, MixedPrecisionPolicy

        if use_dedicated_params:
            raise NotImplementedError(
                "qwen2.5-vl-act has no DMuon support yet; "
                "set optimizer.use_dedicated_params=false."
            )
