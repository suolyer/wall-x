...
class FSDPStrategy(DistributionStrategy):
    ...
    def _wrap_fsdp(
        ...
        model: torch.nn.Module,
        *,
        dmuon_dedicated_params: bool = False,
    ) -> torch.nn.Module:
        ...
        wrapped = model.convert_to_fsdp(
            ...
            use_dedicated_params=dmuon_dedicated_params,
        )
        torch.cuda.empty_cache()
        return wrapped

    def wrap(self, model: torch.nn.Module) -> torch.nn.Module:
        return self._wrap_fsdp(model, dmuon_dedicated_params=False)
...
class DMuonFSDPStrategy(FSDPStrategy):
    ...
    def wrap(self, model: torch.nn.Module) -> torch.nn.Module:
        return self._wrap_fsdp(model, dmuon_dedicated_params=True)
