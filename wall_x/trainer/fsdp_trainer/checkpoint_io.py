...
def load_weights(
    *,
    ...
    aligned_state_dict = reshape_compatible_state_dict(
        filtered_state_dict, model_sd, log_fn=log_fn
    )
    err = model.load_state_dict(aligned_state_dict, strict=False)
...
def _load_fsdp2_or_dmuon_full(
    *,
    ...
    if is_dmuon:
        ...
        model_sd = load_file(safetensors_path)
        import dmuon
    ...
    *,
    ...
    if is_dmuon or is_fsdp2:
        ...
        if is_dmuon or is_main:
            ...
            ref_options = StateDictOptions(full_state_dict=True, cpu_offload=True)
            model_sd = get_state_dict(model, options=ref_options)
            state_dict = reshape_compatible_state_dict(
                state_dict, model_sd, log_fn=log_fn
            )
        else:
            state_dict = {}
        if is_dmuon:
